...
def stratified_sampling_probability_optimized(data, sample_size_dict_1, sample_size_dict_2, group_by_attributes):
    """优化后的分层采样函数(只提升采样处理速率)"""
    random_values = np.random.random(len(data))

    # 计算采样概率
    prob_dict_1 = {}
    prob_dict_2 = {}
    for group, sample_size in sample_size_dict_1.items():
        n = data[group_by_attributes].value_counts()[group]
        prob_dict_1[group] = (sample_size / n) if n > 0 else 0.0
    for group, sample_size in sample_size_dict_2.items():
        n = data[group_by_attributes].value_counts()[group]
        prob_dict_2[group] = (sample_size / n) if n > 0 else 0.0

    probs_1 = data[group_by_attributes].map(prob_dict_1)

    mask_1 = random_values < probs_1

    # 第一次采样
    sampled_df_1 = data[mask_1]
    remaining_df = data[~mask_1]

    # 第二次采样
    random_values_2 = np.random.random(len(remaining_df))
    probs_2 = remaining_df[group_by_attributes].map(prob_dict_2)
    mask_2 = random_values_2 < probs_2

    sampled_df_2 = remaining_df[mask_2]

    return sampled_df_1, sampled_df_2


def uniform_sampling(data, group_column, agg1_column, agg2_column, sample_size_1, sample_size_2,
                     random_state: int = 42):
    """
    根据各组的概率进行无放回的均匀采样，第二次采样从第一次采样的剩余数据中进行
    """
    required_columns = [group_column, agg1_column, agg2_column]
    if not all(col in data.columns for col in required_columns):
        raise ValueError(f"列 {required_columns} 中有不存在于DataFrame中的列")

    group_frequencies = data[group_column].value_counts(normalize=True)

    # 第一个聚合属性的采样量
    group_sample_counts_1 = (group_frequencies * sample_size_1).astype(int)
    remaining_samples_1 = sample_size_1 - group_sample_counts_1.sum()
    if remaining_samples_1 > 0:
        top_groups = group_frequencies.nlargest(remaining_samples_1).index
        for group in top_groups:
            group_sample_counts_1[group] += 1

    # 第二个聚合属性的采样量
    group_sample_counts_2 = (group_frequencies * sample_size_2).astype(int)
    remaining_samples_2 = sample_size_2 - group_sample_counts_2.sum()
    if remaining_samples_2 > 0:
        top_groups = group_frequencies.nlargest(remaining_samples_2).index
        for group in top_groups:
            group_sample_counts_2[group] += 1
    ...
    # 第一个聚合属性的采样
    for group in group_frequencies.index:
        group_df = data[data[group_column] == group]

        count_1 = group_sample_counts_1[group]
        if len(group_df) < count_1:
            raise ValueError(f"分组 '{group}' 的数据量不足以为聚合属性1采样 {count_1} 条。")
        sampled_1 = group_df.sample(n=count_1, random_state=random_state)
        sampled_agg1 = sampled_1[[group_column, agg1_column]].copy()
        sampled_list_agg1.append(sampled_agg1)

        # 第一次采样后的剩余数据
        remaining = group_df.loc[~group_df.index.isin(sampled_1.index)]
        remaining_after_first.append(remaining)

    # 第一次采样的剩余数据
    remaining_data = pd.concat(remaining_after_first, ignore_index=True)

    # 从剩余数据中进行第二个聚合属性的采样
    for group in group_frequencies.index:
        group_df = remaining_data[remaining_data[group_column] == group]

        count_2 = group_sample_counts_2[group]
        if len(group_df) < count_2:
            raise ValueError(f"分组 '{group}' 的剩余数据量不足以为聚合属性2采样 {count_2} 条。")
        sampled_2 = group_df.sample(n=count_2, random_state=random_state + 1)
        sampled_agg2 = sampled_2[[group_column, agg2_column]].copy()
        sampled_list_agg2.append(sampled_agg2)

        # 最终剩余数据
        remaining = group_df.loc[~group_df.index.isin(sampled_2.index)]
        remaining_after_first.append(remaining)

    N1_agg1 = pd.concat(sampled_list_agg1, ignore_index=True)
    N1_agg2 = pd.concat(sampled_list_agg2, ignore_index=True)
    N2 = pd.concat(remaining_after_first, ignore_index=True)

    return N1_agg1, N1_agg2, N2, group_sample_counts_1, group_sample_counts_2

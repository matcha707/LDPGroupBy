...
if __name__ == '__main__':
    ...
    real_result = table.groupby(group_by_attributes)[aggregation_attribute].count()
    print(f"各分组真实计数：{real_result}")
    ...
    for i in range(10):
        try:
            ...
            sample_counts_list.append(sample_restored_counts)

            sample_restored_counts_series = pd.Series(sample_restored_counts)
            relative_errors = (np.abs(sample_restored_counts_series - real_result) / real_result).to_dict()
            relative_errors_list.append(relative_errors)

        except Exception as e:
            print(f"扰动样本数据失败: {e}")

    average_restored_counts = pd.DataFrame(sample_counts_list).mean().to_dict()
    print("估计计数：", average_restored_counts)

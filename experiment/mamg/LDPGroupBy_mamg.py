...
def stratified_sampling_probability_optimized(data, sample_size_dict, group_by_attributes):
    """优化后的分层采样函数"""
    ...
    for group, sample_size in sample_size_dict.items():
        n = data[group_by_attributes].value_counts().get(group, 0)
        prob_dict[group] = (sample_size / n) if n > 0 else 0.0

    probs = data[group_by_attributes].map(prob_dict)

    mask = random_values < probs

    sampled_df = data[mask]
